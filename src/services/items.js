...
export const itemsService = {
  ...
  searchByBarcode: async (query) => {
    const response = await api.get(
      `/items/search/barcode?q=${encodeURIComponent(query)}`
    );
    return response.data.data;
  },

  generateBarcode: async (itemId, format = "CODE128") => {
    const response = await api.post(`/items/${itemId}/barcode`, { format });
    return response.data.data;
  },

  updateBarcode: async (itemId, barcode, format) => {
    const response = await api.patch(`/items/${itemId}/barcode`, {
      barcode,
      format,
    });
    return response.data.data;
  ...
  getBarcodeHistory: async (itemId) => {
    const response = await api.get(`/items/${itemId}/barcode/history`);
    return response.data.data;
  },
  ...
  exportItems: async (format = "csv", params = {}) => {
    try {
      console.log("Export request:", { format, params });
