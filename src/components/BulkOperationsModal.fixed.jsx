...
const BulkOperationsModal = ({ open, onClose, selectedItems = [] }) => {
  ...
  const handleOperationChange = (operationId) => {
    ...
    // If move operation is selected, show the move modal directly
    if (operationId === "move") {
      setShowMoveModal(true);
    }

    // If tag operation is selected, show the tag modal directly
    if (operationId === "tag") {
      setShowTagModal(true);
    }

    // If delete operation is selected, show the delete modal directly
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      switch (operation) {
        case "move":
          // Move operation is now handled by the BulkMoveModal
          setShowMoveModal(true);
          return; // Exit early to prevent closing the modal

        case "tag":
          if (!operationData.tags || operationData.tags.length === 0) {
        ...
        case "delete":
          // Delete operation is now handled by the BulkDeleteModal
          setShowDeleteModal(true);
          return; // Exit early to prevent closing the modal
