...
const ExportModal = ({ open, onClose, onNext, selectedItems = [] }) => {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      // WORKAROUND: Do not include itemIds parameter
      // This avoids the 400 Bad Request error
      // if (selectedItems && selectedItems.length > 0) {
      //   const itemIds = selectedItems.map((item) => item._id || item.id).filter(Boolean);
      //   if (itemIds.length > 0) {
      ...
      if (error.response) {
        if (error.response.status === 403) {
          errorMessage =
            "You don't have permission to export items. Please contact your administrator.";
        } else if (error.response.status === 400) {
          errorMessage =
            "Invalid export parameters. Please check your selection and try again.";
